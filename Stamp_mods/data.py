...
#################################################
@dataclass
class BagDataset_Augmentations(Dataset):
    """A dataset of bags of instances."""
    bags: Sequence[Iterable[Path]]
    """The `.h5` files containing the bags.
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        if self.bag_size:
            ...
            return feats, len(feats)
@dataclass
class BagDataset_Augmentations_test(Dataset):
    """A dataset of bags of instances."""
    bags: Sequence[Iterable[Path]]
    """The `.h5` files containing the bags.
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        num= random.uniform(0,1)
        augs= False 
        #Select the non-augmented tiles for deployment
        for bag_file in self.bags[index]:
            with h5py.File(bag_file, 'r') as f:
                #This should select at every 
                feat= f['feats'][:]
...
def make_dataset(
    *,
    ...
    if add_features:
        ...
        if type=="test":
            return _make_basic_dataset_test(
                bags=bags, target_enc=targets[0], targs=targets[1], bag_size=bag_size)
        if type=="train_alter": #this trains alternating between the augmented and non-augmented features
            return _make_basic_dataset_augs(
                bags=bags, target_enc=targets[0], targs=targets[1], bag_size=bag_size)
...
def _make_basic_dataset_test(
    *,
    ...
    ds = MapDataset(
        zip_bag_targ,
        BagDataset_Augmentations_test(bags, bag_size=bag_size),
        EncodedDataset(target_enc, targs),
    )

    return ds

def _make_basic_dataset_augs(
    *,
    bags: Sequence[Iterable[Path]],
    target_enc: SKLearnEncoder,
    targs: Sequence[Any],
    ...
    ds = MapDataset(
        zip_bag_targ,
        BagDataset_Augmentations(bags, bag_size=bag_size),
        EncodedDataset(target_enc, targs),
    )
