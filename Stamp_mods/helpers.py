    ...
    *,
    ...
    for fold, (train_idxs, test_idxs) in enumerate(folds):
        ...
        if (preds_csv := fold_path/'patient-preds.csv').exists():
            ...
            folds_df = [fold_train_df,fold_test_df]
            learn = _crossval_train(
                fold_path=fold_path, fold_df=folds_df, fold=fold, info=info,
                target_label=target_label, target_enc=target_enc,
                cat_labels=cat_labels, cont_labels=cont_labels) #originally fold_df=fold_train_df
            learn.export()
...
def _crossval_train(
    *, fold_path, fold_df, fold, info, target_label, target_enc, cat_labels, cont_labels
): #Modified to use the remaining data instead of another 20% of the training folds themselves; this is to account for external validation
    """Helper function for training the folds."""
    assert fold_df[0].PATIENT.nunique() == len(fold_df[0])
    assert fold_df[1].PATIENT.nunique() == len(fold_df[1])
    fold_path.mkdir(exist_ok=True, parents=True)

    info['class distribution'][f'fold {fold}'] = {'train folds': {
        k: int(v) for k, v in fold_df[0][target_label].value_counts().items()},
        'val_fold':{
        k: int(v) for k, v in fold_df[1][target_label].value_counts().items()
        }}
    ################################################
    ...
    learn = train(
        bags= all_df.slide_path.values,
        targets=(target_enc, all_df[target_label].values),
        add_features=add_features,
        valid_idxs=all_df.PATIENT.isin(valid_df.PATIENT),
        path=fold_path)
    learn.target_label = target_label
    learn.cat_labels, learn.cont_labels = cat_labels, cont_labels
