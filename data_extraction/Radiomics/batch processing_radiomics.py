...
#Sourced and adapted from the PyRadiomis package

threading.current_thread().name = 'Main'

######################### File variables################################
ROOT = "/mnt/bulk-sirius/asierrabasco/AsierWorkSpace/PyRadiomics/"
PARAMS = os.path.join(ROOT, 'extractor.yaml')  # Parameter file
LOG = os.path.join(ROOT, 'log.txt')  # Location of output log file
INPUTCSV = os.path.join(ROOT, 'test_RADCURE.csv')
OUTPUTCSV = os.path.join(ROOT, 'RADCURE_features.csv')
#########################################################################

######################### Parallel processing variables################################
TEMP_DIR = '_TEMP_feats'
REMOVE_TEMP_DIR = True  # Remove temporary directory when results have been successfully stored into 1 file
NUM_OF_WORKERS = cpu_count() - 10  # Number of processors to use, keep one processor free for other work
...
if __name__ == '__main__':
  ...
  logger.info('Starting parrallel pool with %d workers out of %d CPUs', NUM_OF_WORKERS, cpu_count())
  # Running the Pool
